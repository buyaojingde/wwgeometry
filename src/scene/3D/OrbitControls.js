    ...
    return function update() {
      ...
      if (
        ...
        const offset = (() => {
          ...
          if (moveForward)
            velocity.z -= 200.0 * delta * scope.moveSpeedRatio * scalDelta;
          if (moveBackward)
            velocity.z += 200.0 * delta * scope.moveSpeedRatio * scalDelta;

          if (moveLeft)
            velocity.x -= 200.0 * delta * scope.moveSpeedRatio * scalDelta;
          if (moveRight)
            velocity.x += 200.0 * delta * scope.moveSpeedRatio * scalDelta;
      ...
      const maxHeight = ConfigStructure.height;
      scope.floodHeight = Math.min(
        Math.max(scope.offsetHeight + scope.floodHeight, 0),
        maxHeight
      );
