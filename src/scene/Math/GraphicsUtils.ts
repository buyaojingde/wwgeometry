import { extras, Graphics, Point } from 'pixi.js';
import {
  Box3,
  BoxBufferGeometry,
  BoxGeometry,
...
export function createRingByAngle(outerRadius, innerRadius, angleInRadian, mat?: Material) {
  const shapeGeometry = new RingBufferGeometry(innerRadius, outerRadius, 32 * 8, 1, 0, angleInRadian);
  mat = mat
    ? mat
 ...
 * @param angleValue
 */
export function drawSector(graphics: any, point: any, radius: number, startAngle: number, angleValue: number): void {
  let num2: number = NaN;
  let num3: number = NaN;
  let num4: number = NaN;
  let num5: number = NaN;
...
export const drawDash = (() => {
  ...
  return (
    ...
    const tilingSprite = new extras.TilingSprite(dashedtexture, linelength, lineStyle[0]);
    tilingSprite.x = x0;
    tilingSprite.y = y0;
...
export function drawRotateArrowLeft(innerRadius: number, outerRadius: number, edgeLen: number, mat?: Material): Mesh {
...
export function drawRotateArrowRight(innerRadius: number, outerRadius: number, edgeLen: number, mat?: Material): Mesh {
  const shape: Shape = new Shape();
 ...
 */
export function buildBoundingBoxBySize(size: Vector3, lineMat?: LineBasicMaterial, color?: number): LineSegments {
...
const lineSegmentsMaterial = new LineBasicMaterial({ vertexColors: VertexColors, depthWrite: false });
