import {
  AxesHelper, BoxBufferGeometry, BoxGeometry, Color, DoubleSide,
  Euler, Group, Matrix4, Mesh, MeshPhongMaterial, Shape, ShapeBufferGeometry, ShapeGeometry, Vector2, Vector3,
} from 'three';
...
export default class LianBoTest {

  static testMain() {
    const v0 = new Vector2D(0, 0);
    const v1 = new Vector2D(1, 0);
    const v2 = new Vector2D(0.3, 0.5);
    const v3 = new Vector2D(1, 1);
    const v4 = new Vector2D(0, 1);
    const poly = new Polygon2D([v0,v1,v2,v3,v4]);
    const result = poly.contains(new Vector2D(0.5,0.5));
    console.log(result);
  }
  ...
  public static renderTest() {
    const stage = Scene2D.getInstance()
      .getStage();
  ...
  public static renderBimTest() {
    ...
    const renderEdge = edge => {
      ...
      const matRed: THREE.MeshPhongMaterial = new MeshPhongMaterial(
        { side: DoubleSide, color: new Color(Math.random(), Math.random(), Math.random()) });
  ...
  private static edgesToVector3(edges: any): any {
    return edges.map(item => {
      return {
        startPoint: new Vector3(item.startPoint.X, item.startPoint.Y, item.startPoint.Z),
        endPoint: new Vector3(item.endPoint.X, item.endPoint.Y, item.endPoint.Z),
      };
    });
  }
  ...
  private static edgeMatrix(edges: any): Matrix4 {
    ...
    const vEnd: Vector3 = edges[edges.length - 1].endPoint.clone().sub(edges[edges.length - 1].startPoint);
